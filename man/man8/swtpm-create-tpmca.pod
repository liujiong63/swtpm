...
The environment variables SWTPM_PKCS11_PIN and SWTPM_PKCS11_SO_PIN should be set to hold
the PINs. If they are not set the default PIN 'swtpm-tpmca' will be used.
...
and TPM CA certificate:
 ...
 #> sudo SWTPM_PKCS11_PIN=mypin /usr/share/swtpm/swtpm-create-tpmca \
		--dir /var/lib/swtpm-localca \
		--overwrite \
		--outfile /etc/swtpm-localca.conf \
		--group tss \
